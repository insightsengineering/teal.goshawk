---
title: "Sample Longitudinal Analyses App"
...
output:
  rmarkdown::html_document:
    ...
    toc_float: true
vignette: >
  %\VignetteIndexEntry{Sample Longitudinal Analyses App}
...
# study information: used to label app and in "Source Data" module
# analysis type: if reading data from entimICE then match to entimICE analysis directory name
...
ADLB_SUBSET <- ADLB %>%
  ...
  mutate(
    ...
    AVISITCD = case_when(
      ...
      grepl("WEEK", toupper(AVISIT)) ~ paste("W", trimws(substr(AVISIT, start = 6, stop = str_locate(AVISIT, "DAY") - 1))),
...
# create ADSL metadata for Source Data tab
...
# create ADLB metadata for Source Data tab
        ...
        div(
          ...
          p(tags$ul(
            ...
            h6(
              ...
              The 'Source Data' tab provides the 'Parameter Description' (PARAM). The biomarker/lab selection pull down
          ...
          p(tags$ul(
            ...
            h6(
              "Consider subject #58 has a baseline value of 5 for assay x and the range of assay x across all subjects is 1 to 10. 
             If the the baseline constraint is selected and the value is changed to a range of 7 to 10 t.hen subjects who do not meet
             that condition are removed from the visualizations. Since subject #58 baseline value is 5 for assay x they are one of the 
             subjects who are removed."
            )
          )),
          h5(
          ...
          p(tags$ul(tags$ul(
            h6("The 'Regression Line' option should only be used in conjunction with the 'Treatment Faceting' option.
...
body(app$server)[[length(body(app$server)) + 1]] <- quote(
  observeEvent(input$showAboutModal, {
    showModal(modalDialog(
      title = "About this shiny app: Longitudinal Visualizations",
      tags$p(
        "This shiny app was brought to you by Pharmaverse/NEST."
      ),
      tags$p(
        paste0("The app uses R version: ", R.version$version.string)
      ),
      tags$p(
        paste0("The app uses teal version: ", utils::packageDescription(pkg = "teal", fields = "Version"))
      ),
      easyClose = TRUE
    ))
  })

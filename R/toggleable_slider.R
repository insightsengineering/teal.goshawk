...
#' @rdname toggle_sidebar
toggle_slider_ui_new <- function(id, label) {
  ns <- NS(id)
  tags$div(
    tags$div(
      class = "flex justify-between mb-1",
...
#' @keywords internal
#' @rdname toggle_slider
toggle_slider_server_new <- function(id, initial_state, ...) {
  moduleServer(id, function(input, output, session) {
    selected_state <- reactiveVal(NULL)
    slider_update_state <- reactiveVal(NULL)
    numeric_update_state <- reactiveVal(NULL)
    slider_shown <- reactive(input$toggle %% 2 == 0)
    observeEvent(c(initial_state$min, initial_state$max, initial_state$value), {
      selected_state(
        list(
      ...
      numeric_update_state(list(min = initial_state$value[1], max = initial_state$value[2]))
    })
...
#' @keywords internal
#' @rdname toggle_slider
keep_slider_state_updated <- function(intial_state, varname, paramname, ANL) { # nolint object_name_linter

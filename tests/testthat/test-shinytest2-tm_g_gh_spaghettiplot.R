tm_g_gh_spaghettiplot_driver <- function() {
  init_teal_app_driver(
    data = get_test_data(),
    modules = tm_g_gh_spaghettiplot(
...
test_that("e2e - tm_g_gh_spaghettiplot initializes and renders a plot", {
  skip_if_not_installed("shinytest2")
  skip_if_too_deep(5)
  app_driver <- tm_g_gh_spaghettiplot_driver()
...
test_that(
  "e2e - tm_g_gh_spaghettiplot starts with expected label and encoding selections.",
  {
    skip_if_not_installed("shinytest2")
    skip_if_too_deep(5)
    app_driver <- tm_g_gh_spaghettiplot_driver()
...
describe("e2e - tm_g_gh_spaghettiplot: changing pick changes plot and does not throw validation errors.", {
  skip_if_not_installed("shinytest2")
  skip_if_too_deep(5)

...
test_that("e2e - tm_g_gh_density_distribution_plot initializes and renders a plot", {
...
test_that(
  "e2e - tm_g_gh_density_distribution_plot starts with expected label and encoding selections.",
  {
    skip_if_not_installed("shinytest2")
    skip_if_too_deep(5)
    app_driver <- tm_g_gh_density_distribution_plot_driver()
    ...
    expect_equal(get_teal_picks_slot(app_driver, "xaxis_var", "variables"), "AVAL")
    expect_equal(get_teal_picks_slot(app_driver, "trt_group", "variables"), "ARM")
  }
)

describe("e2e - tm_g_gh_density_distribution_plot: changing pick changes plot and does not throw validation errors.", {
  skip_if_not_installed("shinytest2")
  skip_if_too_deep(5)

  action_mod <- list(
    xaxis_param = list(slot_name = "values", value = "CRP"),
    xaxis_var = list(slot_name = "variables", value = "BASE"),
